...
<body class="no-skin">
						...
						<table style="margin-bottom: 6px;float: left;">
							<tr>
								...
								<td style="vertical-align:top;padding-left:3px;">
									...
									<a class="btn btn-white btn-info btn-bold" onclick="taaReport('确定要审批通过当前页面所有申请吗?')"><i class="ace-icon fa fa-check-square-o bigger-110"></i>批量审批</a>
									<a class="btn btn-white btn-info btn-bold" onclick="taaBackReport('确定要驳回当前页面所有申请吗?');" title="批量驳回" ><i class="ace-icon fa fa-exclamation-triangle red bigger-110"></i>批量驳回</a>
									</c:if>
	...
	<script type="text/javascript">
			...
			bootbox.confirm(msg, function(result) {
				if(result) {
					...
					if(msg == '确定要审批通过当前页面所有申请吗?'){
						...
						$.ajax({
							type: "POST",
							url: '<%=basePath%>erp/taaReport.do?tm='+new Date().getTime(),
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>erp/taaExcel.do?confirmState='+$("#confirmState").val()+'&busiDate='+$("#busiDate").val()+'&USER_DEPART='+$("#SelectedDepartCode").val();
		}
