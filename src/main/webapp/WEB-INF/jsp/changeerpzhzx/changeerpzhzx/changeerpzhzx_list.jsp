...
<!-- 							</div> -->
	...
	<script type="text/javascript">
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>changeerpzhzx/goAdd.do';
			 diag.Width = 750;
			 diag.Height = 455;
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>changeerpzhzx/delete.do?BILL_CODE="+encodeURI(Id);
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>changeerpzhzx/goEdit.do?BILL_CODE='+encodeURI(Id);
			 diag.Width = 750;
			 diag.Height = 455;
			 diag.Modal = true;				//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>changeerpzhzx/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		function setDiv(item){
		    var div = '<li  class="item-grey clearfix" onclick="showDetail(\''+item.BILL_CODE+'\');"><div><label class="inline" style="margin-bottom:5px;"><span class="list-item-value-title">账号注销</span></label></div><div><label class="inline"><span class="list-item-info">单位:&nbsp;</span><span class="list-item-value">'
		        + item.UNIT_CODE
		...
		function showDetail(code){
			  $.ajax({
				  	type: "POST",
		            //提交的网址
		           	url: '<%=basePath%>changeerpzhzx/showDetail.do?BILL_CODE='+code,		      
		            //返回数据的格式
		        	dataType:'json',		          
...
// 		      		    console.log(datas);
		...
		function setDetail(item){
		    var div = '<div class="profile-user-info"><div class="profile-info-row"><div class="profile-info-name">问题：</div><div class="profile-info-value"><span>账号注销</span></div></div><div class="profile-info-row"><div class="profile-info-name">单位：</div><div class="profile-info-value"><i class="fa fa-map-marker light-orange bigger-110"></i><span>'
		        + item.UNIT_CODE		       
		...
		function printf1(){
			...
			var printURL = "PrintReport.jsp?title=" + encodeURIComponent("报表打印")
				...
				+ "&data=" + encodeURIComponent('<%=basePath%>changeerpzhzx/showDetail.do');
			//再新窗口打开这个打印页面
		...
		function printf(Id){
			 ...
			 diag.URL = '<%=basePath%>changeerpzhzx/goPrint.do?BILL_CODE='+encodeURI(Id);
			 diag.Width = 800;
			 diag.Height = 600;
			 diag.Modal = true;				//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>changeerpzhzx/excel.do';
		}
