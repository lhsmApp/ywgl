...
<body class="no-skin">
							...
							<div id="typeDivMain" class="btn-toolbar inline middle no-margin">
								<div data-toggle="buttons" class="btn-group no-margin">
									...
									<label class="btn btn-sm btn-primary active">
										<input type="radio" class="level_select"  name="level_select" value="0"  checked>全部
									</label>
								</div>
							</div>
						</div>
    ...
    var globalFocus_BUSINESS_CODE = null
	    ...
	    var initUrl='<%=basePath%>approvalconfig/getPageList.do';
		$(top.hangge());//关闭加载状态
		...
		function setDetail(item, value){
			...
			var reason=undefined;
			  if(value==1){
					...
					$('#attachment').css("visibility",'visible');
					/* 获取附件信息 */
					 getProAttachment(item.BILL_CODE,"CHANGE_ERP_XTBG");	
			    }else if(value==2){
			    	title=item.BG_NAME;
					reason=item.BG_REASON;
					//$('#attachment').css("visibility",'hidden');
					$('#attachment').css("visibility",'visible');
					/* 获取附件信息 */
					 getProAttachment(item.BILL_CODE,"CHANGE_ERP_JSBG");	
			    }else if(value==3){
			    	title='GRC帐号新增';
					...
					$('#attachment').css("visibility",'visible');
					/* 获取附件信息 */
					 getProAttachment(item.BILL_CODE,"CHANGE_GRC_QXBG");	
			    }else if(value==5){
			    	title='GRC帐号撤销';
					...
					$('#attachment').css("visibility",'visible');
					/* 获取附件信息 */
					 getProAttachment(item.BILL_CODE,"CHANGE_ERP_YHQXBG");	
			    }
			    else if(value==0){					
			    	if(item.BG_NAME==""){
						...
						if(data.rows){
							$.each(data.rows, function(i, item){
							    ...
							    if(value==1||value==2){
							    	...
							    	data.rows[i].BG_REASON=data.rows[i].REASON;
							        html += setDiv(data.rows[i]);
							    }
								$("#tasks").append(html);
						 	});
