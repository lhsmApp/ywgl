											...
											<div class="col-xs-12">
												...
												<div id="zhongxin" style="padding-top: 13px;">
													<div style="margin:10px 0px;">
														<label for="form-field-yhqxbg-report-reason">权限变更申请原因及内容</label>
														<input type="text" name="BG_REASON" id="BG_REASON" class="form-control" placeholder="请输入权限变更申请原因及内容"/>
													</div>
													<div style="margin:10px 0px;">
														<label for="form-field-yhqxbg-report-depart">单位</label>
													...
													<div style="margin:10px 0px;">
														<label for="form-field-yhqxbg-report-user">部门</label>
													...
													<div style="margin:10px 0px;">
														<label for="form-field-yhqxbg-report-dept">申请人</label>
												    ...
												    <div style="margin:10px 0px;">
														<label for="form-field-yhqxbg-report-depart">申请人部门</label>
...
<!-- 														<input type="text" name="EFFECTIVE_DATE" id="EFFECTIVE_DATE" class="form-control" placeholder="请输入联系方式"/> -->
												...
												<div style="margin:10px 12px;">
													<h4>
														<a id="btnAddProInfoAttachment" class="btn btn-success btn-xs"
															onclick="addProAttachmentByType('CHANGE_ERP_YHQXBG')">
	...
	<script type="text/javascript">
	    ...
	    var initUrl='<%=basePath%>changeerpyhqxbg/getPageList.do';
...
// 		}
		...
		function del(Id){
			...
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>changeerpyhqxbg/delete.do?BILL_CODE="+encodeURI(Id);
					$.get(url,function(data){
		...
		function save(){
			...
			$.ajax({
				type: "POST",
				url: '<%=basePath%>changeerpyhqxbg/save.do',
				data:{USER_EMAIL:uesrEmail,USER_TEL:uesrTel,BG_REASON:bgReason,UNIT_CODE:unitCode,DEPT_CODE:deptCode,USER_CODE:uesrCode,USER_DEPT:uesrDept,USER_JOB:uesrJob,USER_CONTACT:uesrContact,EFFECTIVE_DATE:bgyqDate,BILL_CODE:billCode,ADD_ROLE:addRole,DEL_ROLE:delRole},
		    	dataType:'json',
				cache: false,
		...
		function showDetail(code){
			if(event){
			  ...
			  $.ajax({
				  	type: "POST",
		            //提交的网址
		           	url: '<%=basePath%>changeerpyhqxbg/showDetail.do?BILL_CODE='+code,		      
		            //返回数据的格式
		        	dataType:'json',		          
		            success:function(datas){
		      		...
		      		/* 获取提报中附件信息 */
						 getProAttachment("CHANGE_ERP_YHQXBG");
		            } 
		         });
		}
...
// 			$("#selectTree2_input").val(datas.depnameUnit);
		...
		function setDetail(item){
		    var div = '<div class="profile-user-info"><div class="profile-info-row"><div class="profile-info-name">变更名称：</div><div class="profile-info-value"><span>'
		        + 'ERP系统用户权限变更'
		        +'</span></div></div><div class="profile-info-row"><div class="profile-info-name"> 变更原因 及内容：</div><div class="profile-info-value"><span>'
		        +(item.BG_REASON==undefined ?"":item.BG_REASON)
		        ...
		        + item.BILL_CODE	
		        + '</span></div></div><div class="profile-info-row"><div class="profile-info-name">新增角色：</div><div class="profile-info-value"><i class="fa fa-map-marker light-orange bigger-110"></i><span>'
		        + item.ADD_ROLE
		        + '</span></div></div><div class="profile-info-row"><div class="profile-info-name">删除角色：</div><div class="profile-info-value"><i class="fa fa-map-marker light-orange bigger-110"></i><span>'
		        + item.DEL_ROLE
		...
		function printf(Id){
			if(typeof Id== null || Id== "" || Id== undefined) {
			 ...
			 var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title ="ERP用户权限变更申请单打印";
			 diag.URL = '<%=basePath%>changeerpyhqxbg/printf.do?BILL_CODE='+encodeURI(Id);
			 diag.Width = 800;
			 diag.Height = 550;
			 diag.Modal = true;				//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>changeerpyhqxbg/excel.do';
		}

		//搜索
		function tosearch(){
			initList('<%=basePath%>changeerpyhqxbg/getPageList.do');
		}
		//上报
			...
			bootbox.confirm("确定要对单据"+billCode+"进行上报吗?", function(result) {
				if(result) {
				  $.ajax({
					  	type: "POST",
			            //提交的网址
			           	url: '<%=basePath%>approvalconfig/bgReport.do?BUSINESS_CODE=6&BILL_CODE='+encodeURI(billCode),		      
			            //返回数据的格式
			        	dataType:'json',		          
			            success:function(datas){
			            		bootbox.dialog({
