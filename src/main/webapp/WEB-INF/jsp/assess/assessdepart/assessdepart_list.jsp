...
<body class="no-skin">
			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="assessdepart/list.do" method="post" name="Form" id="Form">
						<table style="margin-top:5px;">
							<tr>
								...
								<td style="vertical-align:top;padding-left:2px;">&nbsp;
								 	<select name="ASSESSDEPART_ID" id="ASSESSDEPART_ID">
										<option value="${ASSESSDEPART_ID}" <c:if test="${ASSESSDEPART_ID != ''}">selected</c:if>>本级</option>
										<option value="" <c:if test="${ASSESSDEPART_ID == ''}">selected</c:if>>全部</option>
									</select>
								</td>
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:5px;">	
							<thead>
								<tr>
									...
									<th class="center">关联系统中的单位</th>
									<th class="center">是否是合并单位</th>
								...
								<c:when test="${not empty varList}">
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											...
											<td class="center">${var.LOCAL_DEPART_NAME }</td>
											<td style="width: 80px;" class="center">
												<c:if test="${var.IS_MERGE_DEPART == '0' }"><span class="label label-important arrowed-in">否</span></c:if>
												<c:if test="${var.IS_MERGE_DEPART == '1' }"><span class="label label-success arrowed">是</span></c:if>
											</td>
											<td style="width: 80px;" class="center">
						...
						<table style="width:100%;">
							<tr>
								<td style="vertical-align:top;">
									<a class="btn btn-mini btn-success" onclick="add('${ASSESSDEPART_ID}');">新增</a>
	...
	<script type="text/javascript">
		...
		//去此ID下子级列表
		function goSondict(ASSESSDEPART_ID){
			top.jzts();
			window.location.href="<%=basePath%>assessdepart/list.do?ASSESSDEPART_ID="+ASSESSDEPART_ID;
		};
		...
		//新增
		function add(ASSESSDEPART_ID){
			 top.jzts();
			 var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>assessdepart/goAdd.do?ASSESSDEPART_ID='+ASSESSDEPART_ID;
			 diag.Width = 600;
			 diag.Height = 400;
			 ...
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
					 parent.location.href="<%=basePath%>assessdepart/listTree.do?ASSESSDEPART_ID=${ASSESSDEPART_ID}&dnowPage=${page.currentPage}";
				}
				diag.close();
			 };
			 diag.show();
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>assessdepart/delete.do?ASSESSDEPART_ID="+Id+"&tm="+new Date().getTime();
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>assessdepart/goEdit.do?ASSESSDEPART_ID='+Id;
			 diag.Width = 600;
			 diag.Height = 400;
			 diag.Modal = true;				//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>assessdepart/excel.do';
		}
	</script>
