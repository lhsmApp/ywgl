				    ...
				    <div class="page-content">
					    ...
					    <div class="page-header">
									...
									<span class="label label-xlg label-yellow arrowed-in arrowed-right"
									    id="subTitle" style="margin-left: 2px;"> 税务系统数据导入</span> 
	...
	<script type="text/javascript"> 
	    ...
	    $(document).ready(function () {
    		...
    		$(gridBase_selector).jqGrid({
    			url: '<%=basePath%>taxExternalSystemData/getPageList.do?'
		        ...
		        colModel: [
						    ...
						    { label: '姓名', name: 'USER_NAME', width: 150, editable: true, edittype:'text', editoptions:{maxLength:'20'}},
						    { label: '证照号码', name: 'STAFF_IDENT', width: 180, editable: true, edittype:'text', editoptions:{maxLength:'18'}},
						    ...
						    { label: '应扣缴税额', name: 'ACCRD_TAX', width: 110, sorttype: 'number', align: 'right', searchrules: {number: true}, formatter: 'number',
                ...
                sortable: true,
    			altRows: true, //斑马条纹
    			editurl: '<%=basePath%>taxExternalSystemData/edit.do?'
                +'SelectedCustCol7='+$("#SelectedCustCol7").val()
	            + '&SelectedBusiDate='+$("#SelectedBusiDate").val()
                //+'&SelectedDepartCode='+$("#SelectedDepartCode").val()
                //+'&DepartTreeSource='+DepartTreeSource
    		...
    		if(!(ids!=null && ids.length>0)){//
                ...
                bootbox.confirm(msg, function(result) {
    				if(result) {
    					...
    					$.ajax({
    						type: "POST",
    						url: '<%=basePath%>taxExternalSystemData/deleteAll.do?'
    		                    +'SelectedCustCol7='+$("#SelectedCustCol7").val()
    			                +'&SelectedBusiDate='+$("#SelectedBusiDate").val()
    			                //+'&SelectedDepartCode='+$("#SelectedDepartCode").val()
		                    //+'&DepartTreeSource='+DepartTreeSource
        	...
        	if(!(ids!=null&&ids.length>0)){
                ...
                bootbox.confirm(msg, function(result) {
        			if(result) {
        				...
        				$.ajax({
        					type: "POST",
        					url: '<%=basePath%>taxExternalSystemData/updateAll.do?'
        	                    +'SelectedCustCol7='+$("#SelectedCustCol7").val()
        		                +'&SelectedBusiDate='+$("#SelectedBusiDate").val()
        		                //+'&SelectedDepartCode='+$("#SelectedDepartCode").val()
		                    //+'&DepartTreeSource='+DepartTreeSource
        ...
        function importItems(){
    	   ...
    	   diag.URL = '<%=basePath%>taxExternalSystemData/goUploadExcel.do?'
               +'SelectedCustCol7='+$("#SelectedCustCol7").val()
               +'&SelectedBusiDate='+$("#SelectedBusiDate").val()
               //+'&SelectedDepartCode='+$("#SelectedDepartCode").val()
           //+'&DepartTreeSource='+DepartTreeSource
        ...
        function exportItems(){
        	window.location.href='<%=basePath%>taxExternalSystemData/excel.do?'
                +'SelectedCustCol7='+$("#SelectedCustCol7").val()
                +'&SelectedBusiDate='+$("#SelectedBusiDate").val()
                //+'&SelectedDepartCode='+$("#SelectedDepartCode").val()
            //+'&DepartTreeSource='+DepartTreeSource
