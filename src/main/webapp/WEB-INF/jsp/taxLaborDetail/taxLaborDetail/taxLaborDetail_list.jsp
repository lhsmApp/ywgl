				    ...
				    <div class="page-content">
					    ...
					    <div class="page-header">
									...
									<span class="label label-xlg label-yellow arrowed-in arrowed-right"
									    id="subTitle" style="margin-left: 2px;"> 劳务报酬及个税导入</span> 
	...
	<script type="text/javascript"> 
	    ...
	    $(document).ready(function () {
    		...
    		$(gridBase_selector).jqGrid({
    			url: '<%=basePath%>taxLaborDetail/getPageList.do?'
		        ...
		        colModel: [
						    ...
						    { label: '姓名', name: 'USER_NAME', width: 120, editable: true, edittype:'text', editoptions:{maxLength:'20'}},
						    { label: '证照号码', name: 'STAFF_IDENT', width: 160, editable: true, edittype:'text', editoptions:{maxLength:'18'}},
						    /* { label: '期间', name: 'BUSI_DATE', width: 80,editable: false},
						    { label: '帐套', name: 'BILL_OFF', width: 120,editable: false}, */
						    ...
						    { label: '应发评审费', name: 'GROSS_PAY', width: 110, sorttype: 'number', align: 'right', searchrules: {number: true}, formatter: 'number',
						    	editable: true, edittype:'text', editoptions:{maxlength:'16', number: true}
						    },
						    
						    { label: '个人所得税', name: 'ACCRD_TAX', width: 110, sorttype: 'number', align: 'right', searchrules: {number: true}, formatter: 'number',
						    	editable: true, edittype:'text', editoptions:{maxlength:'16', number: true}
						    },
						   
						    { label: '实发评审费', name: 'ACT_SALY', width: 110, sorttype: 'number', align: 'right', searchrules: {number: true}, formatter: 'number',
						    	editable: true, edittype:'text', editoptions:{maxlength:'16', number: true}
						    },
						    ...
						    { label: '证照号码', name: 'STAFF_IDENT__', width: 60,hidden : true,editable: true,}
						],
                ...
                sortable: true,
    			altRows: true, //斑马条纹
    			editurl: '<%=basePath%>taxLaborDetail/edit.do?'
                +'SelectedCustCol7='+$("#SelectedCustCol7").val()
	            + '&SelectedBusiDate='+$("#SelectedBusiDate").val()
                +'&SelectedDepartCode='+$("#SelectedDepartCode").val()
                //+'&DepartTreeSource='+DepartTreeSource
    		...
    		if(!(ids!=null && ids.length>0)){//
                ...
                bootbox.confirm(msg, function(result) {
    				if(result) {
    					...
    					$.ajax({
    						type: "POST",
    						url: '<%=basePath%>taxLaborDetail/deleteAll.do?'
    		                    +'SelectedCustCol7='+$("#SelectedCustCol7").val()
    			                +'&SelectedBusiDate='+$("#SelectedBusiDate").val()
    			                +'&SelectedDepartCode='+$("#SelectedDepartCode").val()
		                    //+'&DepartTreeSource='+DepartTreeSource
        	...
        	if(!(ids!=null&&ids.length>0)){
                ...
                bootbox.confirm(msg, function(result) {
        			if(result) {
        				...
        				$.ajax({
        					type: "POST",
        					url: '<%=basePath%>taxLaborDetail/updateAll.do?'
        	                    +'SelectedCustCol7='+$("#SelectedCustCol7").val()
        		                +'&SelectedBusiDate='+$("#SelectedBusiDate").val()
        		                +'&SelectedDepartCode='+$("#SelectedDepartCode").val()
		                    //+'&DepartTreeSource='+DepartTreeSource
        ...
        function importItems(){
    	   ...
    	   diag.URL = '<%=basePath%>taxLaborDetail/goUploadExcel.do?'
               +'SelectedCustCol7='+$("#SelectedCustCol7").val()
               +'&SelectedBusiDate='+$("#SelectedBusiDate").val()
               +'&SelectedDepartCode='+$("#SelectedDepartCode").val()
           //+'&DepartTreeSource='+DepartTreeSource
        ...
        function exportItems(){
        	window.location.href='<%=basePath%>taxLaborDetail/excel.do?'
                +'SelectedCustCol7='+$("#SelectedCustCol7").val()
                +'&SelectedBusiDate='+$("#SelectedBusiDate").val()
                +'&SelectedDepartCode='+$("#SelectedDepartCode").val()
            //+'&DepartTreeSource='+DepartTreeSource
