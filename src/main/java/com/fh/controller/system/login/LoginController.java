...
public class LoginController extends BaseController {
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value = "/login_login_app", produces = "application/json;charset=UTF-8")
	@ResponseBody
	public CommonBaseAndList loginByApp() throws Exception {
		PageData pd = new PageData();
		pd = this.getPageData();
		CommonBaseAndList commonBaseAndList=new CommonBaseAndList();
		...
		if (null != KEYDATA && KEYDATA.length == 2) {
			...
			if (pd != null) {
				...
				try {
					...
					commonBase.setCode(-1);
					commonBase.setMessage("身份验证失败！");
				}
			} else {
				commonBase.setCode(-1);
				commonBase.setMessage("用户名或密码有误！");
				logBefore(logger, USERNAME + "登录系统密码或用户名错误");
				FHLOG.save(USERNAME, "登录系统密码或用户名错误");
			}
		} else {
			commonBase.setCode(-1);
			commonBase.setMessage("请输入用户名或密码！");
		}
		commonBaseAndList.setCommonBase(commonBase);
		return commonBaseAndList;
	}
