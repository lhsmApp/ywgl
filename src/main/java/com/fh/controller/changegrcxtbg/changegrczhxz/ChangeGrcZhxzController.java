package com.fh.controller.changegrcxtbg.changegrczhxz;
...
public class ChangeGrcZhxzController extends BaseController {
	...
	Map<String, TmplConfigDetail> Map_SetColumnsListGrczhxz = new LinkedHashMap<String, TmplConfigDetail>();
	/**保存
	 * @param
	 * @throws Exception
	 */
	...
	public @ResponseBody CommonBase save() throws Exception{
		...
		if(null==pd.getString("BILL_CODE")||pd.getString("BILL_CODE").trim().equals("")){
			...
			changegrczhxzService.updateEdit(pd);
			commonBase.setCode(0);
		}	
		if(commonBase.getCode()==0){
			BillnumUtil.updateBillnum(billNumService, BillNumType.ERP_ZHXZ);
		}
		return commonBase;
	}
	...
	public ModelAndView queryList(Page page) throws Exception{
		...
		Map_SetColumnsListGrczhxz.put("BILL_CODE", new TmplConfigDetail("BILL_CODE", "申请单号", "1", false));	
		Map_SetColumnsListGrczhxz.put("UNIT_NAME", new TmplConfigDetail("UNIT_NAME", "申请单位", "1", false));
		Map_SetColumnsListGrczhxz.put("DEPT_NAME", new TmplConfigDetail("DEPT_NAME", "申请部门", "1", false));
		Map_SetColumnsListGrczhxz.put("ACCOUNT_NEW", new TmplConfigDetail("ACCOUNT_NEW", "帐号", "1", false));
		Map_SetColumnsListGrczhxz.put("ACCOUNT_VALIDITY", new TmplConfigDetail("ACCOUNT_VALIDITY", "帐号有效期", "1", false));
		Map_SetColumnsListGrczhxz.put("ACCOUNT_ROLES", new TmplConfigDetail("ACCOUNT_ROLES", "帐号角色", "1", false));
		Map_SetColumnsListGrczhxz.put("ACCOUNT_REASON", new TmplConfigDetail("ACCOUNT_REASON", "新增帐号原因", "1", false));
		Map_SetColumnsListGrczhxz.put("USERNAME",new TmplConfigDetail("USERNAME", "申请人", "1", false));
		Map_SetColumnsListGrczhxz.put("USER_DEPTNAME", new TmplConfigDetail("USER_DEPTNAME", "申请人部门", "1", false));
		Map_SetColumnsListGrczhxz.put("USER_JOB", new TmplConfigDetail("USER_JOB", "申请人岗位", "1", true));
		Map_SetColumnsListGrczhxz.put("USER_CONTACT", new TmplConfigDetail("USER_CONTACT", "联系方式", "1", false));
		Map_SetColumnsListGrczhxz.put("ENTRY_DATE", new TmplConfigDetail("ENTRY_DATE", "申请日期", "1", true));
		Map_SetColumnsListGrczhxz.put("APPROVAL_STATE", new TmplConfigDetail("APPROVAL_STATE", "处理状态", "1", false));
		return mv;
	}
	/**显示变更详情
	 * @param
	...
	public @ResponseBody PageResult2<PageData> getPageList(Page page) throws Exception{
		...
		page.setPd(pd);
		List<PageData> varList = changegrczhxzService.list(page);
		for(PageData p:varList){
			if(null!=p.getString("APPROVAL_STATE")&&!"".equals(p.getString("APPROVAL_STATE"))){
				if(p.getString("APPROVAL_STATE").equals("0")){
					p.put("APPROVAL_STATE", "审批中");
	...
	public ModelAndView goPrint()throws Exception{
		...
		mv.addObject("ReportURL", "static/js/gridReport/grf/changeGrcZhxz.grf");
		mv.addObject("DataURL", "changegrczhxz/PrintZhxz.do?BILL_CODE="+pd.getString("BILL_CODE"));
		mv.addObject("msg", "edit");
		mv.addObject("pd", pd);
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value="/PrintZhxz")
	@ResponseBody 
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value = "/exportGrczhxz")
	public ModelAndView exportGrczhxz(Page page) throws Exception {
		PageData pd = new PageData();
		PageData pd1 = new PageData();
		PageData pd2 = new PageData();
		pd = this.getPageData();
