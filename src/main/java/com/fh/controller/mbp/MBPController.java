...
public class MBPController extends BaseController {
	...
	public ModelAndView list() throws Exception{
		...
		mv.setViewName("mbp/problemAssign/problemAssign_list");
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value="/listProblemInfo")
	public ModelAndView listProblemInfo() throws Exception{
		//if(!Jurisdiction.buttonJurisdiction(menuUrl, "cha")){return null;} //校验权限(无权查看时页面会有提示,如果不注释掉这句代码就无法进入列表页面,所以根据情况是否加入本句代码)
		ModelAndView mv = this.getModelAndView();
		PageData pd = this.getPageData();
	...
	public @ResponseBody PageResult<PageData> getPageList(JqPage page) throws Exception{
		...
		page.setPd(pd);
		List<PageData> varList = mbpService.list(page);	//列出Betting列表
		int records = mbpService.countJqGrid(pd);
	...
	public ModelAndView edit() throws Exception{
		...
		if (type.equals("edit")) {
			...
			if(null != DATA_IDS && !"".equals(DATA_IDS)){
				...
				mbpService.deleteAll(ArrayDATA_IDS);
			}
		}
		mv.addObject("msg","success");
		mv.setViewName("save_result");
	...
	public ModelAndView goEdit()throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		pd = mbpService.findById(pd);	//根据ID读取
		mv.setViewName("jqgrid/jggrid/jggrid_edit");
